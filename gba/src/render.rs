...
///
/// 画面は 30×20 タイル。左壁 1 + フィールド 10 + 右壁 1 の計 12 列を画面中央に
/// 置き (壁 x=9/x=20)、左サイド (x=0..=8) をネクスト 5、右サイド (x=21..=29) を
/// ホールド・スコア表示用に空けておく (HUD)。可視 20 行は画面の縦 20 タイルを
/// 丁度使い切る。
pub(crate) const FIELD_ORIGIN_TX: i32 = 10;
/// 左壁のタイル x 座標。
